...
// (DPR ~2-3x) that needs a few hundred real pixels to look sharp — so we
// request 400px source images as a middle ground between crisp
// thumbnails and payload size. This works for both images and videos,
// since Drive generates a frame preview for videos too.
function resizeThumbnailUrl(url, size = 400) {
...
export async function GET(request) {
  ...
  if (thumbnailLink) {
    try {
      const res = await fetch(resizeThumbnailUrl(thumbnailLink, 400));
      if (res.ok && res.body) {
