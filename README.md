...
4. Trang **Thư viện**: xem lại toàn bộ ảnh/video đã đồng bộ từ mọi tài khoản, nhóm theo ngày đồng bộ, có thể **Xem / Xoá / Chia sẻ** ngay trên web. Ảnh và video trong lưới đều dùng đúng **ảnh thu nhỏ có sẵn của Google Drive** (không tải nguyên bản gốc, kích thước ~100px để tải cực nhanh) để hiển thị nhanh — video có thêm huy hiệu ▶ nhỏ ở góc để phân biệt với ảnh; nếu Drive chưa kịp tạo ảnh thu nhỏ (file vừa đồng bộ xong) thì hiện tạm icon ▶ lớn. Chỉ khi bấm vào xem toàn màn hình mới tải bản đầy đủ. Bấm vào ảnh/video mở khung xem toàn màn hình ngay trong trang (không rời trang) — vuốt trái/phải trong khung xem để chuyển sang ảnh kế tiếp/trước đó, đóng lại thì quay đúng về vị trí đã cuộn tới, không bị nhảy lên đầu trang. Nút **Chia sẻ** tải đúng file ảnh/video gốc về rồi mở hộp thoại chia sẻ của hệ điều hành (Web Share API) — gửi thẳng file thật sang Zalo, Facebook, Messenger... y như chia sẻ từ thư viện ảnh trên máy, không phải gửi link Google Drive.
...
- **File ảnh/video được tải lên theo từng mẩu nhỏ (~4MB), qua server của app.** Lý do: các nền tảng hosting miễn phí như Vercel giới hạn cứng mỗi request gửi lên server tối đa khoảng 4.5MB — đủ cho ảnh nhưng gần như luôn nhỏ hơn video thật. Vì vậy luồng tải lên chia làm 3 bước nhỏ:
  ...
  2. Trình duyệt cắt file thành từng mẩu ~4MB, gửi lần lượt từng mẩu tới `/api/drive/upload/chunk` — route này chuyển tiếp mẩu đó cho Google ngay từ phía server. Làm vậy để mọi request trình duyệt gửi đi đều là cùng gốc với app (same-origin), không phụ thuộc việc Google có "mở cửa" cho trình duyệt gọi thẳng hay không (CORS) — chắc chắn hoạt động trên mọi thiết bị. Nếu 1 mẩu bị lỗi mạng tạm thời, app tự thử lại đúng mẩu đó vài lần trước khi báo lỗi cả file.
...
6. **Cân bằng dung lượng** (trang Tài khoản, cần ít nhất 2 tài khoản): chọn 1 trong 2 kiểu cân bằng — **"Theo % dung lượng"** (tài khoản tổng dung lượng lớn hơn được lưu nhiều hơn theo đúng tỉ lệ, cả hai cùng đầy khoảng bao nhiêu %) hoặc **"Chia đều dữ liệu thật"** (không quan tâm tổng dung lượng mỗi tài khoản bao nhiêu, cứ chuyển file cho tới khi chênh lệch giữa tài khoản chứa ít dữ liệu nhất và nhiều nhất còn trong khoảng 100MB). Bấm "Kiểm tra & lên kế hoạch cân bằng" → app tính toán nên chuyển file nào từ tài khoản đang đầy hơn sang tài khoản còn trống hơn theo đúng kiểu đã chọn, **hiện danh sách cụ thể để bạn xem trước** (tên file, từ tài khoản nào sang tài khoản nào) → bấm "Xác nhận chuyển" mới thật sự thực hiện. Xong việc sẽ có **thông báo kèm âm thanh** báo hoàn tất (khác với thông báo lặng lẽ lúc đồng bộ). Cách hoạt động: server tải file từ tài khoản nguồn và đẩy thẳng sang tài khoản đích (không qua trình duyệt), chỉ xoá bản gốc sau khi bản sao mới đã lưu thành công — nên không có rủi ro mất file giữa chừng. Dù chọn kiểu nào, app luôn tôn trọng dung lượng trống thật sự của mỗi tài khoản — không bao giờ lên kế hoạch chuyển file vào một tài khoản không còn đủ chỗ chứa.
